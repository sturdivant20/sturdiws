...
if __name__ == "__main__":
    ...
    # # grab truth spline
    # truth_file = Path("./data/drone_sim.bin")
    # truth = parse_truth_to_spline(truth_file, 494998.07)
    ...
    for ii, cno in enumerate([40]):  # range(40, 42, 2)):
        new_dir = f"CNo_{cno}_dB"

        for jj in tqdm(
            iterable=[5],  # range(20, 30),
            ...
            #     noise_file = indir / "noise" / f"noise-{jj}.bin"
            #     out_file = Path("./data") / f"SigSim-{kk}.bin"
            #     pool.apply_async(
            #         combine_int16_iq_files, args=(sig_file, noise_file, out_file, 2**29)
            #     )
            # pool.close()
            # pool.join()
